...
enum RouteType
{
    ...
    RouteType_DirectorCut,      //use director's cut replay
...
bool GetMyRoute(int client)
{
    ...
    int accountId = SteamIDToAccountID(steamID);
    int style = gI_Style[client] == -1 ? Shavit_GetBhopStyle(client) : gI_Style[client];
    int track = Shavit_GetClientTrack(client);

    // Build all candidate paths
    char personalPath[PLATFORM_MAX_PATH];
    FormatEx(personalPath, PLATFORM_MAX_PATH, "%s/copy/%d_%s.replay", gS_ReplayFolder, accountId, gS_Map);

    char dcSteamID[64];
    GetClientAuthId(client, AuthId_Steam2, dcSteamID, sizeof(dcSteamID));
    ReplaceString(dcSteamID, sizeof(dcSteamID), ":", "_");

    char dcPath[PLATFORM_MAX_PATH];
    FormatEx(dcPath, PLATFORM_MAX_PATH, "%s/%s/%s_style%d_track%d.replay", gS_DirectorFolder, gS_Map, dcSteamID, style, track);

    char wrPath[PLATFORM_MAX_PATH];
    Shavit_GetReplayFilePath(style, track, gS_Map, gS_ReplayFolder, wrPath);

    RouteType routeType = gRT_RouteType[client];

    if(routeType == RouteType_DirectorCut)
    {
        // Explicit DC mode: only use DC replay
        if(FileExists(dcPath))
        {
            strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, dcPath);
        }
        else
        {
            return false;
    ...
    else if(routeType == RouteType_PersonalReplay)
    {
        if(FileExists(personalPath))
        {
            strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, personalPath);
        }
        else
        {
            return false;
    ...
    else if(routeType == RouteType_ServerRecord || routeType == RouteType_ServerRecordAuto)
    {
        if(FileExists(wrPath))
        {
            strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, wrPath);
        }
        else
        {
            return false;
    ...
    else // RouteType_Auto: DC > Personal > WR
    {
        if(FileExists(dcPath))
        {
            strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, dcPath);
        }
        else if(FileExists(personalPath))
        {
            // Check track matches
            ...
            if(header.iTrack == track)
            {
                strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, personalPath);
            }
            else if(FileExists(wrPath))
            {
                strcopy(gS_ReplayPath[client], PLATFORM_MAX_PATH, wrPath);
            }
            else
...
public void OnPlayerRunCmdPost(int client, int buttons, int impulse, const float vel[3], const float angles[3], int weapon, int subtype, int cmdnum, int tickcount, int seed, const int mouse[2])
{
    ...
    int aheadFrames = gCV_NumAheadFrames.IntValue;
    float velDiff = GetVelocityDifference(client, iClosestFrame);

    // Draw all skipped frames between prevFrame and closestFrame
    // Each frame draws its corresponding lookAhead segment
    int startFrame = gI_PrevFrame[client] + 1;
    int endFrame = iClosestFrame;

    // Player moved backwards (e.g. turned around)
    if(startFrame > iClosestFrame)
    {
        // Reverse: draw from closestFrame up to prevFrame to fill backward gap
        startFrame = iClosestFrame;
        endFrame = gI_PrevFrame[client];
    }

    for(int i = startFrame; i <= endFrame; i++)
    {
        int lookAhead = i + aheadFrames;

        if(lookAhead >= iEndFrame)
        {
            lookAhead = iEndFrame - 1;
        }
        ...
        gA_FrameCache[client].aFrames.GetArray(lookAhead - 1, replay_prevframe, sizeof(frame_t));

        DrawPath(client, replay_prevframe, replay_frame, velDiff);
    }

    gI_PrevFrame[client] = iClosestFrame;

    // 让跳跃标记和路线的超前引导端保持位置同步
    DrawJumpMarkers(client, iClosestFrame + aheadFrames);
}

void DrawPath(int client, frame_t prev, frame_t cur, float velDiff)
{
...
void DrawJumpMarkers(int client, int iClosestFrame)
{
	...
	int newJumpIndex = gA_JumpMarkerCache[client].Length;
    ...
    for(int i = 0; i < gA_JumpMarkerCache[client].Length; i++)
    {
        ...
        if(current.frameNum >= iClosestFrame) // 这里的 iClosestFrame 已经加上了 aheadFrames
        {
            newJumpIndex = current.id;
            break;
        }
    }

    if(newJumpIndex >= gA_JumpMarkerCache[client].Length)
    {
        return;
    }

    // 判断标记目标是否发生了切换
    bool bIndexChanged = (gI_JumpsIndex[client] != newJumpIndex);
    gI_JumpsIndex[client] = newJumpIndex;

    // 如果标记没变，正常走 0.5 秒的性能限制；如果切换了下一个跳点，立刻强制渲染新标记！
    if(!bIndexChanged && gF_Delay[client] && (GetEngineTime() - gF_Delay[client]) < DRAW_DELAY)
    {
        return;
    }
...
void GetClientRouteType(int client, char[] buffer, int length)
{
    switch(gRT_RouteType[client])
    {
        case RouteType_Auto:
        {
            strcopy(buffer, length, "自动");
        }

        case RouteType_DirectorCut:
        {
            strcopy(buffer, length, "导演剪辑版");
        }

        case RouteType_PersonalReplay:
        {
            strcopy(buffer, length, "个人录像");
        }

        case RouteType_ServerRecord:
        {
            strcopy(buffer, length, "服务器纪录");
        }

        case RouteType_ServerRecordAuto:
        {
            strcopy(buffer, length, "服务器纪录 (自动)");
        }
    }
}
...
void GetPathType(int client, char[] buffer, int length)
{
    switch(gI_PathColorIndex[client])
    {
        case -1:
        {
            strcopy(buffer, length, "速度差异");
        }

        default:
        {
            strcopy(buffer, length, "纯色");
        }
    }
}
...
bool CreateMyRouteMenu(int client, int page = 0)
{
    ...
    menu.AddItem("enabled", gB_ShowRoute[client] ? "[X] 启用" : "[ ] 启用");
    ...
    menu.AddItem("pathsettings", "[路径设置]");
    menu.AddItem("jumpmarker", "[跳跃标记]");
...
bool CreatePathSettingsMenu(int client, int page = 0)
{
    ...
    menu.AddItem("enabled", gB_ShowPath[client] ? "[X] 启用" : "[ ] 启用");
    menu.AddItem("-1", "", ITEMDRAW_SPACER);

    char type[32];
    GetPathType(client, type, sizeof(type));

    FormatEx(display, sizeof(display), "路径类型: [%s]", type);
    menu.AddItem("path_type", display);

    if(gI_PathColorIndex[client] != -1)
    {
        menu.AddItem("path_color", "[路径颜色]");
    }

    menu.AddItem("-1", "", ITEMDRAW_SPACER);

    FormatEx(display, sizeof(display), "粗细: [%d]", gI_PathSize[client]);
    menu.AddItem("path_size", display, ITEMDRAW_DISABLED);
    menu.AddItem("increment", "++ 增大路径粗细 ++");
    menu.AddItem("decrement", "-- 减小路径粗细 --");

    FormatEx(display, sizeof(display), "透明度: [%d]", gI_PathOpacity[client]);
    menu.AddItem("path_opacity", display, ITEMDRAW_DISABLED);
    menu.AddItem("opacity_increment", "++ 增大透明度 ++");
    menu.AddItem("opacity_decrement", "-- 减小透明度 --");
...
bool CreateJumpMarkersMenu(int client, int page = 0)
{
    ...
    menu.AddItem("enabled", gB_ShowJumps[client] ? "[X] 启用" : "[ ] 启用");
    menu.AddItem("-1", "", ITEMDRAW_SPACER);

    FormatEx(display, sizeof(display), "大小: [%d]", gI_JumpSize[client]);
    menu.AddItem("marker_size", display, ITEMDRAW_DISABLED);
    menu.AddItem("increment", "++ 增大标记大小 ++");
    menu.AddItem("decrement", "-- 减小标记大小 --");

    menu.AddItem("-1", "", ITEMDRAW_SPACER);

    FormatEx(display, sizeof(display), "[标记颜色]");
    menu.AddItem("marker_color", display);

    FormatEx(display, sizeof(display), "前方跳跃数: [%d]", gI_JumpsAhead[client]);
...
{
    gB_Debug = !gB_Debug;
    ReplyToCommand(client, "调试模式: %s", gB_Debug ? "已启用" : "已关闭");
